...
// SimpleDefenseMatchRule -- tries to join existing instance, puts all players on defenders (team 2).

class SimpleDefenseMatchRule : public MatchRule {
public:
        ...
        for (const auto& inst : mission_instances) {
            if (inst.map_name == "Rot_Redistribution05" &&
                ...
                for (const auto& p : players) {
                    ...
                    result.task_force_assignments[p.session_guid] = 2;
                }
                return result;
            }
        }
