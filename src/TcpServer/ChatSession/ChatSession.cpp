...
void ChatSession::start() {
    ...
    Logger::Log("tcp", "[Chat] Client connected, sessions=%zu\n", g_chat_sessions.size());
    do_read();
}
        ...
        [this, self](std::error_code ec, std::size_t len) {
            if (!ec) {
                ...
                g_chat_sessions.erase(
                    ...
                    g_chat_sessions.end());
                Logger::Log("tcp", "[Chat] Client disconnected, sessions=%zu\n", g_chat_sessions.size());
            }
        });
}
...
void ChatSession::handle_packet(const uint8_t* data, size_t length) {
    ...
    if (packet_type != 0x0073) {
        Logger::Log("tcp", "[Chat] Unknown packet type 0x%04X\n", packet_type);
        return;
    }
    ...
    if (length >= 8) {
        ...
        if (first_field == 0x0473) {
            ...
            if (length >= 8 + 16) {
                ...
                auto info = PlayerRegistry::GetByGuid(guid_hex);
                if (info) {
                    player_name_ = info->player_name;
                    Logger::Log("tcp", "[Chat] Handshake from '%s' (guid=%s)\n",
                        player_name_.c_str(), guid_hex.c_str());
                } else {
                    Logger::Log("tcp", "[Chat] Handshake received, guid=%s not found in registry\n", guid_hex.c_str());
                }
            } else {
                Logger::Log("tcp", "[Chat] Handshake received (guid too short)\n");
            }
            return;
        }
    }

    Logger::Log("tcp", "[Chat] Message received, broadcasting to %zu peer(s)\n",
...
void ChatSession::broadcast(const uint8_t* data, size_t length) {
	...
	// if (pkt.Exists(GA_T::PLAYER_NAME)) {
	// 	isDM = true;
	//
	// 	playerNameDM = pkt.ReadString(GA_T::PLAYER_NAME).value_or("");
	// 	std::string message = player_name_ + " says: " + pkt.ReadString(GA_T::MESSAGE).value_or("");
	// } else {
	// 	std::string message = player_name_ + ": " + pkt.ReadString(GA_T::MESSAGE).value_or("");
	// }
	...
	uint16_t chunk_size = response.size();

	uint16_t length_prefix = static_cast<uint16_t>(chunk_size);
	chunk.push_back(length_prefix & 0xFF);
	chunk.push_back(length_prefix >> 8);

	chunk.insert(chunk.end(), response.begin(), response.end());

	Logger::Log("chat", "Broadcasting raw data:\n");
	for (int i = 0; i < length; i++) {
		Logger::Log("chat", "%02X", data[i]);
	}
	Logger::Log("chat", "\n");
	for (int i = 0; i < chunk.size(); i++) {
		Logger::Log("chat", "%02X", chunk[i]);
	}
	Logger::Log("chat", "\n");
