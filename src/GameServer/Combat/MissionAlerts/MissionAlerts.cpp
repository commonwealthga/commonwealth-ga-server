...
// Only called for the custom CTR rotation variant — on retail Rot_* maps these
// alerts are kismet-driven and firing them here would double them up.
static void PollPointRotationCycle(ATgGame* Game, PerGameState& st) {
	...
	// Rotation banner: ROTATION_BANNER_LEAD_SECS before next activation.
...
void MissionAlerts::Tick() {
	...
	if (isPointRotation) {
		...
		// Retail Rot_* maps fire the rotation banner / activation countdown from
		// their own kismet; the repurposed CTR_* maps have none, so the manual
		// cycle alerts run only when the CTR rotation variant seeded this map.
		if (CtrPointRotation::IsActive()) {
