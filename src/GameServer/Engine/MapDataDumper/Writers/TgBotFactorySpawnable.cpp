...
void WriteTgBotFactorySpawnable(sqlite3* db, AActor* actor,
                                ...
                                int mapObjectId) {
	const char* kSql =
		"INSERT INTO map_tg_bot_factory_spawnable ("
	...
	if (sqlite3_prepare_v2(db, kSql, -1, &stmt, nullptr) != SQLITE_OK) {
		Logger::Log("mapdump", "prepare failed for map_tg_bot_factory_spawnable: %s\n", sqlite3_errmsg(db));
		return;
	}
	BindCommonCols(stmt, actor, mapObjectId, mapName, className);
	if (sqlite3_step(stmt) != SQLITE_DONE) {
		Logger::Log("mapdump", "insert failed for map_tg_bot_factory_spawnable: %s\n", sqlite3_errmsg(db));
	}
