...
void WriteTgDestructibleFactory(sqlite3* db, AActor* actor,
                                const std::string& mapName,
                                const std::string& className,
                                int mapObjectId) {
	const char* kSql =
		"INSERT INTO map_tg_destructible_factory ("
	...
	if (sqlite3_prepare_v2(db, kSql, -1, &stmt, nullptr) != SQLITE_OK) {
		Logger::Log("mapdump", "prepare failed for map_tg_destructible_factory: %s\n", sqlite3_errmsg(db));
		return;
	}
	BindCommonCols(stmt, actor, mapObjectId, mapName, className);
	if (sqlite3_step(stmt) != SQLITE_DONE) {
		Logger::Log("mapdump", "insert failed for map_tg_destructible_factory: %s\n", sqlite3_errmsg(db));
	}
