...
| **Two sources in one bucket do NOT stack — the newcomer displaces** | Measured + Server | In game 2026-08-03: two medics landing Poison Grenades on one target produce a single tick stream, not two. `IsStrongest` gives the rule — an incoming group is dropped only if one already applied has a HIGHER application value, or an equal one that outlasts it; a tie on both falls through to `return true`, so the newcomer replaces. Same-source re-application is a refresh regardless. |
