...
| **Two sources in one bucket do NOT stack, and the NEWEST always wins** | Measured | In game 2026-08-03, two results. Two medics landing Poison Grenades on one target give a single tick stream, not two. And a Healing Grenade landing on someone carrying a nanite heal-over-time removes it and replaces it with its own — then niting them again replaces the grenade's. Both directions displace, so the priority is not gating anything: the grenade's 64 should have lost to a nanite's 86 and does not. Same-source re-application is a refresh. |
| **The priority does NOT block an application** | Measured, and contradicts the code | `IsStrongest` drops an incoming group when one already applied has a higher application value, so the Healing Grenade should never overwrite a nanite. It does. Whatever spares an application from that path is the same unexplained gap behind the burn-refresh result. Until something is seen to block on priority, treat 157 Strongest Wins as newest-wins. **Untested for protections** — `applyStacking` still ranks category 986 shred by priority then lifetime, and nothing has checked whether that is real either. |
| A burn or regen's application rule decides which contends and how | Server + UC | 157 strongest · 156 newest · 836 refresh · 155 separate instances · 874 oldest |
| **The priority often tracks the strength, but not always — and appears unused for contention** | Data | For Poison it is exact: a group's application value equals its per-tick damage (15→15, 55→55, 158→158). For Regeneration it is only approximate — Nanite Restoration System heals 86 a tick at priority 86, but Nanite Enhancement System and Adrenaline Gun also heal 86 a tick at priority **66**, and Healing Grenade heals 64 at 64. So the priority is a ranking the designers set, which usually but not reliably mirrors the number. Do not infer one from the other. Contrast category 986 protection shred, flat 10 across every group, where the lifetime decides instead. |
| **Heal-over-time from different devices contends; their side buffs do not** | Data | Every nanite gun and the Healing Grenade put their heal-over-time in Regeneration/Strongest Wins, so they displace one another — and measurement says whichever landed LAST wins, not the strongest. Each gun's *other* buff sits in its own category (Proximity Damage Buff, Power Pool Buff, General Buff), so those never contend and run on independent timers. |
...
| A thrown mine's damage belongs to the THROWER and takes their modifiers | Measured | In game 2026-08-03: a bomb's detonation damage rose with `Damage Increase: AoE` allocated. This is what justifies classifying a device by its payload — a bomb carries neither radius nor damage itself, both live on the device reached through `device_projectile_id`. Only that chain is followed; a turret reached by `bot_id` stays a separate combatant. |
...
| **Same-source refresh holds for HEALS as well as burns** | measured for burns, assumed for regens by symmetry | the test vehicle is NOT Regeneration - its 15s heal-over-time sits behind a 60s cooldown, so it can never be re-applied while running. It has to be a Nanite Enhancement System, Nanite Restoration System or Adrenaline Gun: 0.5s refire against a 10s heal-over-time. Hold one on a damaged team-mate and see whether the HoT ticks |
...
| **A modifier prop pairs with its base prop by name** | `bench.js` `propList` is a hand-written table - prop 4 Recharge Time is modified by prop 203 Recharge Time Modifier, and so on. The database has no "modifies" column; the pairing rests on the names matching | if the engine scopes a modifier some other way, ungated skills would reach the wrong devices. Checked for the clearest case: Offhand Recharge is ungated in the data (no required skill, no required category, posture None) and resolving it by "device has a Recharge Time" lands on exactly the 49 Offhand-slot devices its description names, so nothing observable separates the two readings |
