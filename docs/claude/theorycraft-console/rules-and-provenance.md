...
| **A device is classified by what it THROWS as well as what it is** | a bomb has neither radius nor damage of its own; both live on the payload reached through `device_projectile_id` | only the projectile chain is followed, so a turret reached by `bot_id` stays separate. If the engine treats a deployed mine's damage as the mine's rather than the thrower's, the carrier's AOE skills would not apply after all |
